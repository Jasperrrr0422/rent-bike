...
class LocationDataView(APIView):
    ...
    """
    def get(self, request):
        locations = Location.objects.all()  # 获取所有位置记录
        serializer = LocationSerializer(locations, many=True)  # 序列化数据
        return Response(serializer.data)  # 返回序列化后的数据
    

class VehicleRentalCountView(APIView):
    def get(self, request):
        # 查询每辆车的租用次数，并确保没有租用记录的车辆返回0
